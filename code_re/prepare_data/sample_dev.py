...
def prepare_tst():
    ...
    for file_name in os.listdir(train_data_path):
        ...
        if file_name.endswith('ann'):
            continue
        with open(os.path.join(train_data_path, file_name)) as ftxt:
            lines = "".join([line for line in ftxt])

        entity_dict = defaultdict(dict)
        with open(os.path.join(train_data_path, file_name.replace('txt', 'ann'))) as fann:
            for line in fann:
        ...
        data_list = []
        for e1_id_, e1_info_ in sorted_entity_dict:
            nearby_entity_dict = [(eid_, einfo_) for eid_, einfo_ in sorted_entity_dict if
                                  0 < einfo_['e_idx'] - e1_info_['e_idx'] <= 100]
