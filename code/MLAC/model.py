...
class MultiLevelAttentionCNN(nn.Module):
    def __init__(self, opt, padding_idx=0, embedding=None):
        ...
        # self.conv = nn.Conv1d(in_channels=self.kd, out_channels=self.dc, kernel_size=3)
        self.conv = nn.Conv2d(1, self.dc, (1, self.kd), (1, self.kd), bias=True)
...
class NovelDistanceLoss(nn.Module):
    ...
    def forward(self, wo, rel_weight, in_y):
        wo_norm = F.normalize(wo)  # [b, dc]
        b, dc = wo_norm.size()
        wo_norm_tile = wo_norm.unsqueeze(1).repeat(1, self.nr, 1)  # [b, nr, dc]
        batched_rel_w = F.normalize(rel_weight).unsqueeze(0).repeat(b, 1, 1)  # [b, nr, dc]
        all_distance = torch.norm(wo_norm_tile - batched_rel_w, 2, 2)  # [b, nr, 1]
        mask = one_hot(in_y, self.nr, 1000, 0)
